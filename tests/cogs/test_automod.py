...
class TestAutomodAdd:
    ...
    async def test_account_age_without_threshold(self) -> None:
        ...
        call_args = interaction.response.send_message.call_args
        assert "account_age_minutes" in call_args.args[0]
...
class TestAccountAgeEdgeCases:
    ...
    def test_threshold_seconds_zero_returns_false(self) -> None:
        """threshold_seconds = 0 は falsy なのでマッチしない。"""
        cog = _make_cog()
        rule = _make_rule(
            rule_type="account_age",
            threshold_seconds=0,
    ...
    def test_max_threshold_seconds(self) -> None:
        """最大閾値 (1209600秒 = 14日) で新しいアカウントにマッチ。"""
        cog = _make_cog()
        rule = _make_rule(
            rule_type="account_age",
            threshold_seconds=1209600,
            pattern=None,
        )
...
class TestOnMemberJoinEdgeCases:
    ...
    async def test_multiple_non_matching_rules(self) -> None:
        ...
        rule2 = _make_rule(
            rule_id=2, rule_type="account_age", threshold_seconds=86400, pattern=None
        )
...
class TestSendLogEmbed:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("banned", "[AutoMod] User Banned", 0xE74C3C),
            ("kicked", "[AutoMod] User Kicked", 0xE67E22),
            ("timed_out", "[AutoMod] User Timed_out", 0xF1C40F),
        ],
    )
...
class TestOnMessage:
    ...
    async def test_non_message_post_rule_skipped(self) -> None:
        ...
        msg.author = member
        rule = _make_rule(
            rule_type="account_age", pattern=None, threshold_seconds=86400
        )
        with (
            patch(
                "src.cogs.automod.get_enabled_automod_rules_by_guild",
...
class TestAutomodAddTimingRules:
    ...
    @pytest.mark.asyncio
    async def test_successful_add_with_account_age_minutes(self) -> None:
        cog = _make_cog()
        interaction = _make_interaction()
...
class TestCheckRoleCount:
    ...
    def _role(self, role_id: int, name: str) -> MagicMock:
    ...
    def test_no_target_role_ids_returns_false(self) -> None:
        """target_role_ids 未設定 → マッチしない。"""
        cog = _make_cog()
        rule = _make_rule(
            rule_type="role_count",
            pattern=None,
    ...
    def test_one_below_boundary_no_match(self) -> None:
        ...
        member = _make_member()
        member.roles = [self._role(100, "A"), self._role(200, "B")]
        matched, _ = cog._check_role_count(rule, member)
        assert matched is False

    def test_reason_includes_role_names(self) -> None:
        """reason にマッチしたロール名が含まれる。"""
        cog = _make_cog()
        rule = _make_rule(
            rule_type="role_count",
            pattern=None,
        ...
        member = _make_member()
        member.roles = [self._role(100, "VIP"), self._role(999, "Other")]
        matched, reason = cog._check_role_count(rule, member)
        assert matched is True
        assert "VIP" in reason

    def test_non_target_roles_ignored(self) -> None:
        """target に含まれないロールはカウントしない。"""
        cog = _make_cog()
        rule = _make_rule(
            rule_type="role_count",
            pattern=None,
            threshold_seconds=2,
            target_role_ids="100,200",
        )
        member = _make_member()
        # target外のロール999を持っていてもカウントされない
        member.roles = [self._role(100, "A"), self._role(999, "X")]
        matched, _ = cog._check_role_count(rule, member)
        assert matched is False
...
class TestOnMemberUpdateRoleCount:
    ...
    async def test_matching_role_count_bans(self) -> None:
        cog = _make_cog()
        role1 = self._role(100, "Role1")
        role2 = self._role(200, "Role2")
...
class TestAutomodAddRoleCount:
    ...
    @pytest.mark.asyncio
    async def test_role_count_without_target_roles(self) -> None:
        cog = _make_cog()
        interaction = _make_interaction()
        await cog.automod_add.callback(
            cog,
        ...
        call_args = interaction.response.send_message.call_args
        assert "target_roles" in call_args.args[0]
    ...
    async def test_successful_add_role_count(self) -> None:
        ...
        with patch(
            ...
            assert create_kwargs["target_role_ids"] == "100,200,300"
...
class TestAutomodListRoleCountDisplay:
    ...
    async def test_shows_role_count_threshold(self) -> None:
        ...
        with patch(
            ...
            embed = call_kwargs["embed"]
            field_values = [f.value for f in embed.fields]
            assert any("5個以上" in v for v in field_values)
            assert any("5ロール" in v for v in field_values)
