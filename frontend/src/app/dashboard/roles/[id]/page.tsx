...
export default function RolePanelDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchData() {
    const res = await fetch(`${API_BASE}/rolepanels/${id}`).then((r) => r.json())
  ...
  async function handleSave(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`${API_BASE}/rolepanels/${id}`, {
        method: 'PUT',
  ...
  async function handlePost() {
    const res = await fetch(`${API_BASE}/rolepanels/${id}/post`, {
      method: 'POST',
    })
    if (res.ok) {
  ...
  async function handleCopy() {
    const res = await fetch(`${API_BASE}/rolepanels/${id}/copy`, {
      method: 'POST',
    })
    if (res.ok) {
  ...
  async function handleDelete() {
    ...
    try {
      await fetch(`${API_BASE}/rolepanels/${id}`, {
        method: 'DELETE',
      })
      setDeleteOpen(false)
  ...
  async function handleDeleteItem(itemId: number) {
    const res = await fetch(`${API_BASE}/rolepanels/${id}/items/${itemId}`, {
      method: 'DELETE',
    })
    if (res.ok) {
  ...
  async function handleMoveItem(itemId: number, direction: 'up' | 'down') {
    const res = await fetch(`${API_BASE}/rolepanels/${id}/items/${itemId}/move`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function handleAddItem(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    setAddingItem(true)
    try {
      const res = await fetch(`${API_BASE}/rolepanels/${id}/items`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          emoji: newEmoji || null,
