...
export default function AutoModSettingsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await fetch(`${API_BASE}/automod/settings`).then((r) => r.json())
  ...
  useEffect(() => {
    async function loadConfig(guildId: string) {
      ...
      try {
        const res = await fetch(`${API_BASE}/automod/settings?guild_id=${guildId}`)
        if (res.ok) {
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    setSaved(false)
    try {
      await fetch(`${API_BASE}/automod/settings`, {
        method: 'PUT',
