...
export default async function AutoModPage() {
  const res = await apiFetch<{ rules: AutoModRule[]; guilds: GuildsMap; channels: ChannelsMap }>(
    `${API_BASE}/automod/rules`
  )
    ...
    },
    {
      ...
      accessor: (row) => (
        <div className="flex items-center gap-2">
          <ToggleButton
            endpoint={`${API_BASE}/automod/rules/${row.id}/toggle`}
          ...
          <DeleteButton endpoint={`${API_BASE}/automod/rules/${row.id}/delete`} />
        </div>
      ),
    },
  ]
