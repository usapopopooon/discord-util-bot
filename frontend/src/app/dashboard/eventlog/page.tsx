...
export default function EventLogPage() {
  ...
  async function fetchData() {
    const res = await fetch(`${API_BASE}/eventlog`).then((r) => r.json())
    ...
    },
    {
      ...
      accessor: (row) => (
        <div className="flex items-center gap-2">
          <ToggleButton endpoint={`${API_BASE}/eventlog/${row.id}/toggle`} enabled={row.enabled} />
          <DeleteButton endpoint={`${API_BASE}/eventlog/${row.id}/delete`} />
        </div>
      ),
    },
  ]
