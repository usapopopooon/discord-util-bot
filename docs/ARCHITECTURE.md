...
class AutoModRule(Base):
    ...
    is_enabled: Mapped[bool]              # 有効/無効
    action: Mapped[str]                   # "ban" | "kick" | "timeout"
    pattern: Mapped[str | None]           # マッチパターン (username_match 用)
    use_wildcard: Mapped[bool]            # 部分一致 (username_match 用)
    threshold_seconds: Mapped[int | None] # 全時間ベースルール統一カラム (秒)
    timeout_duration_seconds: Mapped[int | None]  # timeout アクション時の期間 (秒)
    required_channel_id: Mapped[str | None]  # 投稿必須チャンネル (intro 系ルール用)
    created_at: Mapped[datetime]
...
class AutoModConfig(Base):
    ...
    log_channel_id: Mapped[str | None] # BAN/KICK/Timeout ログ送信先チャンネル ID
    intro_check_messages: Mapped[int]  # チャンネル履歴フォールバック件数 (default 50, 0=無効)
```
...
class AutoModLog(Base):
    ...
    action_taken: Mapped[str]          # 実行されたアクション ("banned" | "kicked" | "timed_out")
...
| `account_age` | アカウント作成から N 分以内 (最大 14日) | `on_member_join` |
...
| `vc_without_intro` | 指定チャンネル未投稿で VC 参加 | `on_voice_state_update` |
| `msg_without_intro` | 指定チャンネル未投稿でメッセージ投稿 | `on_message` |
...
2. ギルドの有効ルールを取得
3. 対象ルールタイプの条件をチェック
4. マッチ → BAN / キック / タイムアウト実行 (メンバー情報を事前保存)
5. ログを DB に保存
6. `AutoModConfig` にログチャンネルが設定されていれば Embed を送信

#### ログ Embed
BAN/KICK/Timeout 実行時、`AutoModConfig.log_channel_id` にリッチ Embed を送信:
- ユーザー情報 (名前、ID、アバター)
- アクション (BANNED/KICKED/TIMED_OUT)
- 適用ルール (ID + タイプ)
- 理由
...
| `/automod/settings` | AutoMod 設定 (ログチャンネル、履歴チェック件数) |
| `/eventlog` | イベントログ設定一覧 |
| `/eventlog/new` | イベントログ設定作成 |
| `/joinrole` | 入室時ロール設定一覧 |
| `/joinrole/new` | 入室時ロール設定作成 |
| `/activity` | Bot アクティビティ設定 |
| `/settings` | 設定画面 (タイムゾーン、パスワード変更等) |
| `/settings/timezone` | タイムゾーン設定 (POST) |
...
def _handle_sigterm(_signum: int, _frame: FrameType | None) -> None:
    """Railway 等のシャットダウン時に SIGTERM を受信"""
...
### 12. タイムゾーン設定 (`utils.py` + `SiteSettings` モデル)

タイムゾーンオフセットは以下の優先順位で決定:
1. **DB (`SiteSettings.timezone_offset`)** — Web 管理画面から設定可能
2. **環境変数 `TIMEZONE_OFFSET`** — DB に未設定時のフォールバック (デフォルト 9 = JST)

Bot (`setup_hook`) と Web (`lifespan`) の両方で起動時に DB から読み込み、
`set_timezone_offset()` でランタイムに反映。

```python
# utils.py
def format_datetime(dt: datetime | None, fmt: str = "%Y-%m-%d %H:%M", *, fallback: str = "-") -> str:
    """DB またはenv のタイムゾーンオフセットでローカル日時にフォーマット。"""
```

適用箇所: `ticket_view.py` (トランスクリプト)、`templates/` (Web 管理画面)、`eventlog.py` (Embed 内日時)
